...
class auditoriaDao
{
    ...
    public static function listadoCompuestoAuditoriaEmpresa($filtro,$tipo,$estado,$consulta,$idempresa)
    {
        $status ="";
         switch ($filtro) {
            case 'az':
    ...
    public static function numeroCapitulosElegidos($idgrupo,$idauditoria)
    {
        ...
        foreach ($data_table_1 as $key => $value) {
            ...
            if(intval($objItem->getIdgrupo_plantilla_item()) == intval($idgrupo) && $estado != 1 ){
                $contador++;
            }
        }
        return $contador;
